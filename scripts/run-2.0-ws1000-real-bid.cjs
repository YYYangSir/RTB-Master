...
const caseId = args.get('case') ?? 'TC-WS1000-003';
const wsCount = Number(args.get('ws') ?? 1000);
const bidderCount = Number(args.get('bidders') ?? 20);
const minutes = Number(args.get('minutes') ?? 10);
const intervalSec = Number(args.get('interval') ?? 10);
...
async function setupAuction(admin) {
  ...
  const product = await must('/products', {
    ...
    body: { name: `2.0 1000 WS 真实出价拍品 ${stamp}`, description: `${caseId} 自动化测试拍品` },
  });
  const room = await must('/live-rooms', {
    method: 'POST',
    headers: authHeaders(admin),
    body: { title: `2.0 1000 WS 真实出价直播间 ${stamp}` },
  });
...
async function connectOne(auctionId, metrics, index) {
  ...
  socket.on('bidAccepted', (payload) => {
    ...
    if (price < lastPrice) metrics.priceErrorCount += 1;
    metrics.lastPriceByClient[index] = price;
    metrics.latestSnapshot = payload;
    const top = payload?.leaderboard?.[0];
    if (top && top.amountCent !== price) metrics.leaderboardErrorCount += 1;
  });
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < bidderCount; i += 1) {
      bidders.push(await login(`2.0 ${caseId} 出价用户 ${String(i + 1).padStart(2, '0')}`, 'BIDDER'));
    }
    ...
    while (Date.now() < endAt) {
      const candidates = bidders.filter((user) => user.id !== leaderUserId);
      const bidder = candidates[Math.floor(Math.random() * candidates.length)] ?? bidders[0];
      ...
      if (result.ok) {
        ...
        amountCent += incrementCent;
      } else {
        bidFailures += 1;
        const detail = await request(`/auctions/${auction.id}`);
        if (detail.ok) {
          leaderUserId = detail.data.leaderUserId;
          amountCent = (detail.data.currentPriceCent ?? amountCent) + incrementCent;
        }
      }
    ...
    const wsTop = metrics.latestSnapshot?.leaderboard?.[0] ?? null;
    const expectedMessages = connected * acceptedBids;
    ...
    const finalConsistent = Boolean(
      top &&
      wsTop &&
      detail.currentPriceCent === top.amountCent &&
      detail.currentPriceCent === metrics.latestSnapshot?.currentPriceCent &&
      detail.leaderUserId === top.userId &&
      detail.leaderUserId === metrics.latestSnapshot?.leaderUserId &&
      top.userId === wsTop.userId &&
      top.amountCent === wsTop.amountCent
    );
    ...
    const result = {
      ...
      status: (
        ...
        percentile(bidLatencies, 0.95) <= 1000 &&
        metrics.priceErrorCount === 0 &&
        metrics.leaderboardErrorCount === 0 &&
        finalConsistent &&
        healthAfter
      ) ? 'PASS' : 'FAIL',
      ...
      acceptedBids,
      bidFailures,
      bidStatusCounts: statusCounts,
      bidP95Ms: Number(percentile(bidLatencies, 0.95).toFixed(2)),
      bidMaxMs: Number(Math.max(0, ...bidLatencies).toFixed(2)),
      expectedMessages,
      receivedMessages: metrics.received,
      arrivalRate: Number(arrivalRate.toFixed(4)),
      avgBroadcastLatencyMs: Number(average(metrics.latencies).toFixed(2)),
      p95BroadcastLatencyMs: Number(percentile(metrics.latencies, 0.95).toFixed(2)),
      p99BroadcastLatencyMs: Number(percentile(metrics.latencies, 0.99).toFixed(2)),
      maxBroadcastLatencyMs: Number(Math.max(0, ...metrics.latencies).toFixed(2)),
      priceErrorCount: metrics.priceErrorCount,
      leaderboardErrorCount: metrics.leaderboardErrorCount,
      ...
      finalLeaderboard: {
        ...
        topAmountCent: top?.amountCent ?? null,
      },
      finalWebSocket: {
        currentPriceCent: metrics.latestSnapshot?.currentPriceCent ?? null,
        leaderUserId: metrics.latestSnapshot?.leaderUserId ?? null,
        topUserId: wsTop?.userId ?? null,
        topAmountCent: wsTop?.amountCent ?? null,
      },
      finalConsistent,
      healthAfter,
    };
    ...
    appendFileSync('直播竞猜测试执行记录.md', [
      ...
      `| 出价用户 | ${result.bidderCount} |`,
      `| 真实出价 | 成功 ${result.acceptedBids}，失败 ${result.bidFailures}，接口 P95 ${result.bidP95Ms}ms |`,
      `| 广播到达 | ${result.receivedMessages}/${result.expectedMessages}，到达率 ${result.arrivalRate} |`,
      `| 广播延迟 | 平均 ${result.avgBroadcastLatencyMs}ms，P95 ${result.p95BroadcastLatencyMs}ms，P99 ${result.p99BroadcastLatencyMs}ms，最大 ${result.maxBroadcastLatencyMs}ms |`,
      `| 当前价错误 | ${result.priceErrorCount} |`,
      `| 排行榜错误 | ${result.leaderboardErrorCount} |`,
      `| 最终一致性 | ${result.finalConsistent ? '通过' : '失败'} |`,
      `| API 健康 | ${result.healthAfter ? '通过' : '失败'} |`,
      '',
      result.status === 'PASS'
        ? '结论：`TC-WS1000-003` 通过，1000 在线下 20 个真实出价用户触发的出价广播及时性和最终一致性达到 2.0 验收标准。'
        : '结论：`TC-WS1000-003` 未通过，需要根据出价接口、广播到达率、广播延迟或最终一致性定位问题。',
      '',
    ].join('\n'));
